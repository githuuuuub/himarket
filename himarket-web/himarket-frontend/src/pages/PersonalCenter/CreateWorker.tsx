...
function CreateWorker() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "file">("file");
  ...
  const parseZipAndFillForm = async (file: File) => {
    ...
    try {
      ...
      const manifestEntry = zip.file(/^([^/]+\/)?manifest\.json$/i);
      if (manifestEntry.length > 0) {
        const content = await manifestEntry[0].async("string");
        setOverviewContent(content);
        setActiveTab("overview");
        try {
          const manifest = JSON.parse(content);
          if (manifest.name && !form.getFieldValue("name")) form.setFieldValue("name", manifest.name);
          if (manifest.description && !form.getFieldValue("description")) form.setFieldValue("description", manifest.description);
          if (manifest.icon && !form.getFieldValue("icon")) form.setFieldValue("icon", manifest.icon);
        } catch { /* invalid JSON */ }
      } else {
        setOverviewContent(null);
        setActiveTab("file");
  ...
  const handleSelectFile = useCallback(async (path: string) => {
    ...
    try { const content = await zipEntry.async("string"); setFileContent(content); }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await createAsset({ name: values.name, type: "WORKER", description: values.description, icon: values.icon, config });
  ...
  const renderFilePreview = () => {
    ...
    if (fileBinary) return <div className="text-gray-400 text-center py-16 text-sm">二进制文件，不支持预览</div>;
    if (fileContent === null) return <div className="text-gray-400 text-center py-16 text-sm">文件加载失败</div>;
        ...
        <div className="flex-1 overflow-auto bg-white h-full flex flex-col relative">
          ...
          {mdRawMode ? (
            ...
            <div className="flex-1 overflow-auto px-6 pb-6 pt-8">
              <div className="markdown-body text-sm"><MarkdownRender content={fileContent} /></div>
            </div>
          )}
        </div>
      );
      ...
      <div className="py-8 flex flex-col gap-4">
        ...
        <div className="flex-shrink-0">
          ...
          <div className="flex items-center gap-4 mb-3">
            <div className="w-16 h-16 rounded-xl flex-shrink-0 flex items-center justify-center bg-green-50 border border-gray-200">
              <UserOutlined className="text-2xl text-green-600" />
            </div>
            <div className="flex-1 min-w-0">
              <h1 className="text-xl font-semibold text-gray-900 mb-1">创建 Worker</h1>
              <div className="text-sm text-gray-400">上传工作流执行器包，自动解析元数据预填信息</div>
            </div>
          </div>
            ...
            >
              {!zipFileName ? (
                <div className="py-3">
                  ...
                  <p className="text-xs text-gray-400">上传后自动解析元数据并预填表单信息</p>
                </div>
              ) : (
          ...
          <div className="flex-1 min-w-0">
            <div className="bg-white rounded-lg overflow-hidden flex flex-col" style={{ height: "calc(100vh - 340px)", minHeight: 460, border: "1px solid #f0f0f0" }}>
              <div className="flex gap-6 px-4 pt-3 flex-shrink-0" style={{ borderBottom: "1px solid #f0f0f0" }}>
                {hasOverview && (
                  <button className={`pb-2 text-sm font-medium transition-colors border-b-2 ${activeTab === "overview" ? "text-blue-600 border-blue-600" : "text-gray-500 border-transparent hover:text-gray-700"}`} onClick={() => setActiveTab("overview")}>
                    Overview
                  </button>
                )}
                <button className={`pb-2 text-sm font-medium transition-colors border-b-2 ${activeTab === "file" ? "text-blue-600 border-blue-600" : "text-gray-500 border-transparent hover:text-gray-700"}`} onClick={() => setActiveTab("file")}>
                  File
                </button>
              </div>
              ...
              {activeTab === "overview" && hasOverview && (
                <div className="flex-1 overflow-auto p-6">
                  <pre className="text-xs bg-gray-50 p-4 rounded-lg overflow-auto whitespace-pre-wrap" style={{ fontFamily: "'Menlo', 'Monaco', 'Courier New', monospace" }}>
                    {overviewContent}
                  </pre>
                </div>
              )}
          ...
          <div className="w-full lg:w-[360px] flex-shrink-0 space-y-3">
            <div className="bg-white rounded-xl overflow-hidden shadow-sm" style={{ border: "1px solid #e8eaef" }}>
              <div className="px-4 py-3" style={{ borderBottom: "1px solid #edeef3" }}>
                ...
                <span className="text-xs text-gray-400 ml-2">从元数据自动解析</span>
              </div>
              <div className="px-4 py-4">
                <Form form={form} layout="vertical" requiredMark="optional" size="middle">
                  <Form.Item name="name" label="Worker 名称" rules={[{ required: true, message: "请输入名称" }, { max: 128, message: "不超过128字符" }]}>
                    <Input placeholder="输入 Worker 名称" />
                  </Form.Item>
                  <Form.Item name="description" label="描述">
                    <Input.TextArea placeholder="描述该工作流执行器的功能和用途" autoSize={{ minRows: 2, maxRows: 4 }} />
                  </Form.Item>
              ...
              <div className="px-4 py-3" style={{ borderTop: "1px solid #edeef3" }}>
                <Button type="primary" onClick={handleSubmit} loading={submitting} block size="middle">创建资产</Button>
                <Button onClick={() => navigate("/personal-center")} block className="mt-2" size="middle">取消</Button>
              </div>
            </div>
          </div>
        </div>
